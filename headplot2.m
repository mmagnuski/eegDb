...
%   
   ...
   spline_file = arg1;
   %%%%channels to plot MZ
   elecsinfo = find(strcmp('elecs', varargin));
   if elecsinfo
       elecs = varargin{elecsinfo+1};
       varargin(elecsinfo:elecsinfo+1)=[];
   else
       elecs=[];
   end
  ...
  %choosing channels to plot MZ
  if exist('indices') && ~isempty(elecs);
      plotelecs = intersect(1:length(indices), elecs);
      indices = 1:length(plotelecs);
      Xe = Xe(plotelecs);
      Ye = Ye(plotelecs);
      Ze = Ze(plotelecs);
      G = G(plotelecs, plotelecs);
      gx = gx(:, plotelecs);
      newElect = newElect(plotelecs,:);
      ElectrodeNames = ElectrodeNames(plotelecs,:);     
  end
...
if exist('center') ~= 1, center = [0 0 0]; %disp('Using [0 0 0] for center of head mesh'); 
end;
newPOS = POS(index1,:);
