...
%   'axis'            - the axis handle in which to draw {default: gca}
%
% Plot detail options:
...
MAXDEFAULTSHOWLOCS = 100;% if more channels than this, don't show electrode locations by default
...
if nargs > 2
    ...
    for i = 1:2:length(varargin)
        ...
        switch Param
            ...
            case 'gridscale'
                GRID_SCALE = Value;
                if isstr(GRID_SCALE) | GRID_SCALE ~= round(GRID_SCALE) | GRID_SCALE < 10
                    error('''gridscale'' value must be integer > 10.');
                end
...
if ~strcmpi(STYLE,'blank') % if draw interpolated scalp map
    ...
    set(haxis,'Xlim',[-rmax rmax]*AXHEADFAC,'Ylim',[-rmax rmax]*AXHEADFAC);
    % specify size of head axes in haxis
    ...
    if strcmpi(STYLE,'grid')                     % plot grid only
        ...
        %
        handle=imagesc('Parent',haxis,'XData',Xi,'YData',Yi,'CData',gridcolors); % plot grid with explicit colors
        axis(haxis,'square')
    ...
    elseif strcmp(STYLE,'contour')                     % plot surface contours only
        [cls chs] = contour(haxis,Xi,Yi,ZiC,CONTOURNUM,'k');
    ...
    elseif strcmp(STYLE,'both')  % plot interpolated surface and surface contours
        if strcmp(SHADING,'interp')
            tmph = surface(Xi*unsh,Yi*unsh,zeros(size(Zi))-0.1,Zi,...
                'EdgeColor','none','FaceColor',SHADING,'Parent',haxis);
        else % SHADING == 'flat'
        ...
        if ~PMASKFLAG
            ...
            [cls chs] = contourf(haxis,Xi,Yi,ZiC,0,'k');
    ...
    elseif strcmp(STYLE,'straight') | strcmp(STYLE,'map') % 'straight' was former arg
        ...
        if strcmp(SHADING,'interp') % 'interp' mode is shifted somehow... but how?
            tmph = surface(Xi*unsh,Yi*unsh,zeros(size(Zi)),Zi,'EdgeColor','none',...
                'FaceColor',SHADING,'Parent',haxis);
        else
    ...
    elseif strcmp(STYLE,'fill')
        [cls chs] = contourf(haxis,Xi,Yi,Zi,CONTOURNUM,'k');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Set color axis  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %
    caxis(haxis,[amin amax]); % set coloraxis
    ...
    if strcmp(ELECTRODES,'labelpoint') |  strcmp(ELECTRODES,'numpoint')
        text(-0.6,-0.6, ...
            [ int2str(length(Rd)) ' of ' int2str(length(tmpeloc)) ' electrode locations shown'],'Parent',haxis);
        text(-0.6,-0.7, [ 'Click on electrodes to toggle name/number'],'Parent',haxis);
        tl = title(haxis,'Channel locations');
...
if ~strcmpi(STYLE,'grid')                     % if not plot grid only
    ...
    if strcmp(CONVHULL,'on') %%%%%%%%% mask outside the convex hull of the electrodes %%%%%%%%%
        ...
        ringh2= patch(ringy,ringx,ones(size(ringy)),BACKCOLOR,'edgecolor','none','Parent',haxis); hold(haxis,'on')
        ...
        if ~strcmpi(STYLE,'blank')
            ringh= patch(ringx,ringy,0.01*ones(size(ringx)),BACKCOLOR,'edgecolor','none','Parent',haxis); hold(haxis,'on')
        end
        % plot(ry*rmax,rx*rmax,'b') % debugging line
    end
    
    %f1= fill(rin*[rx rX],rin*[ry rY],BACKCOLOR,'edgecolor',BACKCOLOR); hold(haxis,'on')
    ...
    if headrad > 0                         % if cartoon head to be plotted
        ...
        if ~isstr(HEADCOLOR) | ~strcmpi(HEADCOLOR,'none')
            ringh= patch(headx,heady,ones(size(headx)),HEADCOLOR,'edgecolor',HEADCOLOR,'Parent',haxis); hold(haxis,'on')
        end
        ...
        %   ry(k) = ry(k)*(1+hwidth);
        % end
        % f3= fill(hin*[rx rX],hin*[ry
        % rY],HEADCOLOR,'edgecolor',HEADCOLOR); hold(haxis,'on')
        ...
        if ~isstr(HEADCOLOR) | ~strcmpi(HEADCOLOR,'none')
            plot3([basex;tiphw;0;-tiphw;-basex]*sf,[base;tip-tipr;tip;tip-tipr;base]*sf,...
                ...
                'Color',HEADCOLOR,'LineWidth',HLINEWIDTH,'Parent',haxis);                 % plot nose
            plot3(EarX*sf,EarY*sf,2*ones(size(EarX)),'color',HEADCOLOR,'LineWidth',HLINEWIDTH,'Parent',haxis)    % plot left ear
            plot3(-EarX*sf,EarY*sf,2*ones(size(EarY)),'color',HEADCOLOR,'LineWidth',HLINEWIDTH,'Parent',haxis)   % plot right ear
        end
    end
    ...
    %
    plotax = haxis;
    axis(haxis,'square')                                           % make plotax square
    axis(haxis,'off')
    ...
    % axes(textax);
    axis(haxis,'square')                                           % make textax square
    ...
    axis(haxis,'equal');
    set(haxis, 'xlim', [-0.525 0.525]); set(plotax, 'xlim', [-0.525 0.525]);
    set(haxis, 'ylim', [-0.525 0.525]); set(plotax, 'ylim', [-0.525 0.525]);
    ...
    if strcmp(ELECTRODES,'on')   % plot electrodes as spots
        if isempty(EMARKER2CHANS)
            hp2 = plot3(y,x,ones(size(x))*ELECTRODE_HEIGHT,...
                EMARKER,'Color',ECOLOR,'markersize',EMARKERSIZE,'linewidth',EMARKERLINEWIDTH,'Parent',haxis);
            ...
            hp2b = plot3(y(mark2chans),x(mark2chans),ones(size((mark2chans)))*ELECTRODE_HEIGHT,...
                EMARKER2,'Color',EMARKER2COLOR,'markerfacecolor',EMARKER2COLOR,'linewidth',EMARKER2LINEWIDTH,'markersize',EMARKERSIZE2,'Parent',haxis);
        end
        %
    ...
    elseif strcmp(ELECTRODES,'labelpoint')
        ...
        for i = 1:size(labels,1)
            hh(i) = text(double(y(i)+0.01),double(x(i)),...
                ...
                'set(gco, ''string'', tmpstr); clear tmpstr;'],'Parent',haxis );
        end
        %
    ...
    try,
        if strcmpi(STYLE,'blank') % if mark-selected-channel-locations mode
            for kk = 1:length(1:length(x))
                if Values(kk) == 3
                    hp2 = plot3(y(kk),x(kk),ELECTRODE_HEIGHT,EMARKER,'Color', [0 0 0], 'markersize', EMARKERSIZE1CHAN,'Parent',haxis);
                elseif Values(kk) == 2
                    hp2 = plot3(y(kk),x(kk),ELECTRODE_HEIGHT,EMARKER,'Color', [0.5 0 0], 'markersize', EMARKERSIZE1CHAN,'Parent',haxis);
                elseif Values(kk) == 1
                    hp2 = plot3(y(kk),x(kk),ELECTRODE_HEIGHT,EMARKER,'Color', [1 0 0], 'markersize', EMARKERSIZE1CHAN,'Parent',haxis);
                elseif strcmpi(ELECTRODES,'on')
                    hp2 = plot3(y(kk),x(kk),ELECTRODE_HEIGHT,EMARKER,'Color', ECOLOR, 'markersize', EMARKERSIZE,'Parent',haxis);
                end
            end
        end
    catch, end;
    ...
    if ~isempty(DIPOLE)
        ...
        if PLOT_DIPOLE
            for index = 1:size(DIPOLE,1)
                hh = plot(haxis,DIPOLE(index, 1), DIPOLE(index, 2), '.');
                set(hh, 'color', DIPCOLOR, 'markersize', DIPSCALE*30);
                hh = line( [DIPOLE(index, 1) DIPOLE(index, 1)+DIPOLE(index, 3)]', ...
                    [DIPOLE(index, 2) DIPOLE(index, 2)+DIPOLE(index, 4)]',[10 10],'Parent',haxis);
...
if strcmpi(DRAWAXIS, 'on')
    axes('position', [0 0.85 0.08 0.1],'Parent',hparent);
    axis(haxis,'off');
    coordend1 = sqrt(-1)*3;
    coordend2 = -3;
    ...
    line([5 5+round(real(coordend1))]', [5 5+round(imag(coordend1))]', 'color', 'k','Parent',haxis);
    line([5 5+round(real(coordend2))]', [5 5+round(imag(coordend2))]', 'color', 'k','Parent',haxis);
    if round(real(coordend2))<0
        text( 5+round(real(coordend2))*1.2, 5+round(imag(coordend2))*1.2-2, '+Y','Parent',haxis);
    else text( 5+round(real(coordend2))*1.2, 5+round(imag(coordend2))*1.2, '+Y','Parent',haxis);
    end;
    if round(real(coordend1))<0
        text( 5+round(real(coordend1))*1.2, 5+round(imag(coordend1))*1.2+1.5, '+X','Parent',haxis);
    else text( 5+round(real(coordend1))*1.2, 5+round(imag(coordend1))*1.2, '+X','Parent',haxis);
    end;
    set(haxis, 'xlim', [0 10], 'ylim', [0 10]);
end;
