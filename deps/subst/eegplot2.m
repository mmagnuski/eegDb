% eegplot2() - Scroll (horizontally and/or vertically) through multichannel data.
...
%
% Additional keywords:
%    'labels'     - [cell of strings] labels to use in marking the data
%                   These are displayed in a drop-down menu and allow the
%                   user to mark the data in a variety of ways.
...
function [outvar1] = eegplot2(data, varargin) % p1,p2,p3,p4,p5,p6,p7,p8,p9)

% TODOs:
% [X] 'LineSmoothing' on as default when plotting eeg signal :)
% [ ] change so that signal is in bigger axis
% [X] option to add tag types on the fly


% Defaults (can be re-defined):

DEFAULT_PLOT_COLOR = { [0 0 1], [0.7 0.7 0.7]};         %#ok<NASGU> % EEG line color
try 
icadefs; %% MMagnuski - changed this so that it does not produce warnings
...
%catch
%		disp('eegplot2() error: calling convention {''key'', value, ... } error'); return;
%end;	
...
% ---------------------------------
defdowncom   = 'eegplot2(''defdowncom'',   gcbf);'; % push button: create/remove window
defmotioncom = 'eegplot2(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
defupcom     = 'eegplot2(''defupcom'',     gcbf);';
defctrldowncom = 'eegplot2(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
...
try g.title; 		    catch, g.title		= ['Scroll activity -- eegplot2()']; 	end;
...
if ~femp(g, 'linesmoothing') || ~ischar(g.linesmoothing) ||...
       sum(strcmp(g.linesmoothing, {'on', 'off'})) == 0
   g.linesmoothing = 'on';
end % MMagnuski
...
% Background axis (MMagnuski: Vat iz thiz?)
% --------------- 
...
if isstr(g.eloc_file) || isstruct(g.eloc_file)  % Read in electrode names
  ...
  eegplot2('setelect', g.eloc_file, ax1);
end;
...
posbut(23,:) = [ 0.0464   0.065    0.0385    0.0339 ]; % << 10
posbut(24,:) = [ 0.2297    0.065    0.0385    0.0339 ]; % >> 10
posbut(25,:) = [ 0.025    0.95    0.07    0.045 ]; % hBox
posbut(1:24,1) = posbut(1:24,1)+0.2;
...
'Callback',['global in_callback;', ...
            'if isempty(in_callback);in_callback=1;', ...
            '    try eegplot2(''drawp'',23);', ...
            '        clear global in_callback;', ...
            '    catch error_struct;', ...
            '        clear global in_callback;', ...
            '        error(error_struct);', ...
            '    end;', ...
            'else;return;end;']); % MZakrzewska
...
'string','-',...
'FontSize',8,...
'Callback','eegplot2(''draws'',2)');

% MMagnuski
% long callback in string.
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%
% MMagnuski's modification %%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%
pos_rejcol = [0.45 0.9 0.33 0.06];
g.choose_rejcol = uicontrol(figh, 'Style', 'popupmenu',...
  'Units', 'normalized', 'Position', pos_rejcol,...
  'String', g.labels, 'Callback',...
  @choose_rejcol_callb);

pos_addrej = [0.80 0.9 0.05 0.06];
uicontrol(figh, 'Style', 'pushbutton',...
  'Units', 'normalized', 'Position', pos_addrej,...
  'String', '+', 'FontSize', 14, 'Callback',...
  @add_rejcol_callb);
...
uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'eegplot2(''noui'');');
...
% Scale Eye %%%%%%%%%
timestring = ['[OBJ1,FIG1] = gcbo;',...
        'eegplot2(''scaleeye'',OBJ1,FIG1);',...
	'clear OBJ1 FIG1;'];
m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);

% Title %%%%%%%%%%%%
uimenu('Parent',m(1),'Label','Title','Callback','eegplot2(''title'')')
...
uimenu('Parent',m(6),'Label','Load .loc(s) file',...
  'Callback','eegplot2(''loadelect'');')

% Zooms %%%%%%%%
zm = uimenu('Parent',m(2),'Label','Zoom off/on');
commandzoom = [ 'set(gcbf, ''windowbuttondownfcn'', [ ''zoom(gcbf,''''down''''); eegplot2(''''zoom'''', gcbf, 1);'' ]);' ...
...
uimenu('Parent',zm,'Label','Zoom off', 'separator', 'on', 'callback', ...
 ...
 'clear tmpg;' ]);
uimenu('Parent',figh,'Label', 'Help', 'callback', 'pophelp(''eegplot2'');');

% Events %%%%%%%%
zm = uimenu('Parent',m(2),'Label','Events');
complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
              'tmpg.plotevent = ''on'';' ...                  
              'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot2(''drawp'', 0);'];
comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
              'tmpg.plotevent = ''off'';' ...                  
              'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot2(''drawp'', 0);'];
comeventmaxstring   = [ 'tmpg = get(gcbf, ''userdata'');' ...
            'tmpg.plotevent = ''on'';' ...
            'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot2(''emaxstring'');']; % JavierLC      
comeventleg  = [ 'eegplot2(''drawlegend'', gcbf);'];
...
set(figh, 'WindowKeyPressFcn',     @(hObj, evnt) eegplot_readkey_new(hObj, evnt, g.hBox));
...
eegplot2('scaleeye', [], gcf);
if strcmp(lower(g.scale), 'off')
  eegplot2('scaleeye', 'off', gcf);
end;
...
elseif p1 == 23
 ...
 elseif p1 == 24
    g.time = g.time + 10; % >> 10 add 10 windows % MZakrzewska
end
...
for i = 1:g.chans
    if strcmpi(g.plotdata2, 'on')
         tmpcolor = [ 1 0 0 ]; %#ok<NASGU>
    else tmpcolor = g.color{mod(i-1,length(g.color))+1}; %#ok<NASGU>
    end;
    ...
    if isfield(g, 'eloc_file') && ...
        ...
        tmpcolor = [ .85 .85 .85 ];
        plot( data(g.chans-i+1, lowlim:highlim) - meandata(g.chans-i+1) ...
            + i*g.spacing + (g.dispchans+1)*(oldspacing-g.spacing)/2 ...
            + g.elecoffset * (oldspacing - g.spacing), 'color', ...
            tmpcolor, 'clipping','on')
        
        % WHAT does this do? (plot lines where bad channels are?):
        plot(1, mean(  data(g.chans-i+1, lowlim:highlim) - ...
            meandata(g.chans-i+1) + i*g.spacing + (g.dispchans+1)...
            *(oldspacing-g.spacing)/2 + g.elecoffset*...
            (oldspacing-g.spacing), 2  ), '<r', 'MarkerFaceColor',...
            'r', 'MarkerSize', 6);
    end
...
%        keyboard;  
    if (isfield(g, 'eloc_file') && ...
           ...
           plot(  data( g.chans-i+1, lowlim:highlim) - ...
...
set(ax1, 'ylim',[g.elecoffset*g.spacing (g.elecoffset+g.dispchans+1)*g.spacing] );
 ...
 if g.children ~= 0
	...
	eegplot2( 'drawp', p1, p2, g.children);
	figure(figh);
 end;	  
          ...
          if g.trialstag ~= -1 % epoched data
              ...
              if ~isempty(indices)
                  ...
                  if size(g.winrej,2) > 2
                      tmpcols  = g.winrej(indices,3:5); % takes out colors from winrej
                  else tmpcols  = g.wincolor; % else - assumes default color
                  end;
                  ...
                  for tmpi = 1:length(tmpwins1)
                      ...
                      set(h, 'EdgeColor', 'none') % MMagnuski change: 'none' instead of getting color from face
                  end;
              end;
          else
...
% MMagnuski CHECK - later change to plot also mark labels
% draw events if any
% ------------------
...
set(gcf, 'UserData', g);
eegplot2('updateslider', fig);
eegplot2('drawp',0);	
eegplot2('scaleeye', [], fig);
return;
...
eegplot2('setelect',[ inputpath inputname ],AXH0);
return;
...
if g.incallback
    % MMagnuski CHECK:
    % realtime edition of winrej (check later if this is OK)
...
case 'defupcom'
    ...
    if ~isempty(g.winrej)', ...
        if g.winrej(end,1) == g.winrej(end,2) % remove unitary windows
            g.winrej = g.winrej(1:end-1,:); % MMagnuski check - is '= []' better?
        else
...
case 'defdowncom'
    ...
    ax1 = findobj('tag','backeeg','parent',fig); 
    % MMagnuski: what is 'backeeg'??
    tmppos = get(ax1, 'currentpoint'); % MMagnuski: here the current cursor position is probed
    if strcmp(get(fig, 'SelectionType'),'normal');
        ...
        if g.incallback ~= 1 % interception of nestest calls
            ...
            if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                ...
                set(fig,'UserData', g);
                eegplot2('drawp', 0);  % redraw background
            end;
        end;
    end;      
otherwise
    error(['Error - invalid eegplot2() parameter: ',data])
end  
end

% MMagnuski added:
function choose_rejcol_callb(hObject,eventdata) %#ok<INUSD>
% 
% option 1:
% get value from hObject
% check in g what color is associated to this value
...
for index=1:length(vals)
	...
	[mintmp, indextmp] = min(minvals(bintmp)); %#ok<ASGLU>
