% eeg_interp2() - interpolate data channels
%
...
function EEG = eeg_interp2(EEG, bad_elec, method)
    ...
    if isstruct(bad_elec)
        ...
        [tmp, tmpchan] = setdiff_bc( lab2, lab1); %#ok<*ASGLU>
        ...
        badchans  = bad_elec;
        goodchans = 1:EEG.nbchan;
        goodchans(badchans) = [];
        
        EEG.data = EEG.data(goodchans, :, :);
    ...
    datachans = find(nonemptychans(goodchans));
    goodchans = goodchans( nonemptychans(goodchans) );
    badchans  = badchans( nonemptychans(badchans) );
    if isempty(badchans), return; end;
    ...
    elseif strcmpi(method, 'spacetime') % 3D interpolation, works but x10 times slower
        ...
        tmpdata = reshape(EEG.data, numel(EEG.data),1);
        zbad = 1:pnts;
