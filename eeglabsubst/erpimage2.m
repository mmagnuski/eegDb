...
%
%   'keepaxis'        =  prevent erpimage from deleting current axis. Defaulf: false
%
...
percentileflag = NO;
erp_ptiles = 1;
keepaxis = false;    % keep axis option  MZ/MM edit
...
if nargin > 6
    ...
    while a < nargin % for each remaining Arg
        ...
        elseif strcmpi(Arg,'erp_grid')
            erp_grid = YES;
        elseif strcmp(Arg, 'keepaxis') % MZ edit
            keepaxis = true;
...
if strcmpi(NoShow, 'no')
    if Colorbar == YES    
        if keepaxis % MZ/MM edit
            ...
            if strcmp(uni, 'normalized')
                axcb=axes('Position',...
                    [pos(1)+pos(3)+0.02 pos(2) ...
                    0.03 pos(4)]);
            elseif strcmp(uni, 'pixels')
                axcb=axes('Units', 'pixels', 'Position',...
                    [pos(1)+pos(3)+10 pos(2) 20 pos(4)], ...
                    'Parent', gcf);
            end
            clear uni
        else
...
if Erpflag == YES & strcmpi(NoShow, 'no')
    ...
    elseif keepaxis %% MMagnuski & MZakrzewska EDIT
        if strcmp('pixels', get(ax1, 'Units'))
            ax2=axes('Units', 'pixels', 'Position',...
                [gcapos(1) gcapos(2)-35 gcapos(3) 35], ...
                'Parent', gcf, 'visible', 'off');
        elseif strcmp('normalized',get(ax1, 'Units'))
            ax2=axes('Units', 'normalized', 'Position',...
                [gcapos(1) gcapos(2)-0.05 gcapos(3) 0.05], ...
                'Parent', gcf, 'visible', 'off');
        end
    else
        ax2=axes('Position',...
