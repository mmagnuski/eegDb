...
% TODOs:
% [ ] add dropdown menu for choosing type of rejection
% [ ] set how this can be defined 'from outside'
% [ ] default - reject, maybe, ?
% [ ] 'LineSmoothing' on as default when plotting eeg signal :)
% [ ] LATER: change so that signal is in bigger axis
...
try 
    icadefs; %% MMagnuski - change this so that it does not produce warnings
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   try g.labcol; g.wincolor = g.labcol(1,:);       catch, g.labcol = [252 177 158; 254 239 156; 196 213 253]./255; g.wincolor = g.labcol(1,:); end; % MMagnuski
...
u(22) = uicontrol('Parent',figh, ...
  ...
  uicontrol(figh, 'Style', 'popupmenu',...
